...
import os, sys, pathlib, datetime, time, pandas
...
# get regional data 
r_df = pandas.read_csv(url_regioni, index_col=['denominazione_regione'], usecols=['nuovi_positivi', 'denominazione_regione'])
r_df = clear_df(r_df)
...
p_df = pandas.read_csv(url_province, index_col=['denominazione_provincia'], usecols=['denominazione_provincia', 'totale_casi'])
p_df = clear_df(p_df)
